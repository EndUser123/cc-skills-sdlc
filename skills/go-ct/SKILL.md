...
name: go-ct
version: 3.0.0
description: "Execute a task from user input, plan file, or tasks.json queue and drive\
  \ it to PR-ready completion. Handles intent parsing, task selection, worktree enforcement,\
  \ verification, simplification, 7-pass review, and local artifact generation. Uses\
  \ the shared enforce layer for phase gate tracking. Not for architecture, design,\
  \ or refactoring \u2014 use /planning, /design_1.0, or /refactor instead."
category: execution
enforcement: strict
workflow_steps:
...
hooks:
  ...
  - matcher: ^/go
    hooks:
    - type: command
      command: python "$CLAUDE_PLUGIN_ROOT"/skills/go-ct/hooks/PreToolUse_go_invocation_receipt.py
      description: Write invocation receipt to phase ledger
  Stop:
  - matcher: .*
    hooks:
    - type: command
      command: python "$CLAUDE_PLUGIN_ROOT"/skills/go-ct/hooks/Stop_enforce_gate.py
...
Worktree root: `P:\worktrees\go-ct-task-{TS}` where `{TS}` is a timestamp. Each task gets its own isolated worktree.
...
**Workflow Execution:**
- Executable via `scripts/go_ct_executor.py`
- Phase gates are flag files: `.worktree-ready_{RUN_ID}`, `.task-selected_{RUN_ID}`, etc.
- State persisted as JSON in `.claude/.artifacts/{terminal_id}/go/`
- Subagent dispatch via `Agent` tool with `isolation: "worktree"`

**DO NOT use:**
- LangGraph (over-engineering for linear sequential workflows)
- In-memory state (multi-terminal requires file-based persistence)
- Hardcoded paths (use env vars: `$CLAUDE_PLUGIN_ROOT`, `$TERMINAL_ID`)
