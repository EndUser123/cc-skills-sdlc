...
**MANDATORY SEQUENCE:** Step 0 (Contract + Worktree) -> Step 0.5 (Discovery) -> Worktree Check -> Task Selection -> Classify -> Dispatch -> Verify -> Simplify -> 7-Pass Review -> QA Verification -> PR Artifacts -> Loop Check
...
- `<promise>SPAWN_FALSIFIER</promise>` — Step 9.8 (conditional, opt-in `GO_FALSIFICATION_ENABLE=1`): `falsification-request_{RUN_ID}.json` written, `.falsification-pending_{RUN_ID}` set, and a harness-created disposable attack worktree exists. The main-loop Claude must:

  1. Read `$GO_STATE_DIR/falsification-request_{RUN_ID}.json` — contains `authoritative_worktree`, `head_revision`, `base_revision`, `changed_files`, task `objective`/`acceptance_criteria`/`scope_in`/`scope_out`, `budget` (max_attempts, timeout_seconds, total_elapsed_seconds), `attack_worktree` path, `result_path`, `request_digest`, `session_id`, `agent_identity`.
  2. Validate the request exists and `.falsification-pending_{RUN_ID}` is present. If either is missing, do NOT proceed — the run state is inconsistent.
  3. Invoke the attacker Agent in the **disposable attack worktree** (`attack_worktree` field in the request). The attacker receives `tools: [Read, Grep, Glob, Bash, Edit, Write]` — **it may mutate the attack worktree** to create tests/disposable modifications.
  4. Prohibit edits to the **authoritative task worktree** (the `authoritative_worktree` field, read-only). The attacker must not read or write anything there.
  5. The attacker must write a structured result at the path in `result_path`. The result must match schema `go.falsification-result.v1` with verdict in `{FALSIFIED, NOT_FALSIFIED_WITHIN_BUDGET, INCONCLUSIVE, HARNESS_FAILURE, BUDGET_EXHAUSTED}`. A `FALSIFIED` verdict requires at least one counterexample with `claim_falsified`, `command`, `expected_result`, `actual_result`, `output`, **and `ledger_entry_id`** (references the command-ledger entry proving the reproduction command actually executed). Counterexamples without a ledger entry are rejected. Reproduction commands MUST be run through `run_constrained_command()` inside the attack worktree so the harness records stdout/stderr digests, exit code, timestamps, and timeout status in the run-scoped command ledger (`falsification-command-ledger_{RUN_ID}.jsonl`). Prose-only claims without executable reproduction are rejected.
  6. Re-invoke the orchestrator: `GO_RUN_ID="$RUN_ID" python "<skill_root>/scripts/orchestrate.py" --falsification-resume "$RUN_ID"`.
  7. Failure to spawn the attacker or write a valid structured result is a **harness failure** — write a `HARNESS_FAILURE` result at the result path with the error detail, then resume.
  8. Terminal verdicts (`FALSIFIED`, `HARNESS_FAILURE`, `INCONCLUSIVE`, `BUDGET_EXHAUSTED`) automatically resolve the session pointer so G4/G5 do not continue looping.

  See the attacker contract at `skills/go/scripts/falsification_gate.py` for schema definitions, budget enforcement, and cleanup semantics.
