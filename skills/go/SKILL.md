...
- `<promise>SPAWN_FALSIFIER</promise>` — Step 9.8 (conditional, opt-in `GO_FALSIFICATION_ENABLE=1`): `falsification-request_{RUN_ID}.json` written, `.falsification-pending_{RUN_ID}` set, and a harness-created disposable attack worktree exists. The main-loop Claude must:
  ...
  1. Read `$GO_STATE_DIR/falsification-request_{RUN_ID}.json` — contains `authoritative_worktree`, `head_revision`, `base_revision`, `changed_files`, task `objective`/`acceptance_criteria`/`scope_in`/`scope_out`, `budget` (max_attempts, timeout_seconds, total_elapsed_seconds), `attack_worktree` path, `result_path`, `request_digest`.
